...
exports.handler = async (event) => {
  ...
  const senderName = clean(auth.display_name) || 'Anna-Maria';
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(subject)}</title></head><body style="margin:0;background:#050505;color:#d7d7d7;font-family:Helvetica,Arial,sans-serif;line-height:1.55"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#050505;padding:32px 16px"><tr><td align="center"><table role="presentation" width="100%" style="max-width:720px;border:1px solid #222;background:#0b0b0b" cellspacing="0" cellpadding="0"><tr><td style="padding:28px 30px;border-bottom:1px solid #202020"><img src="https://melankoliaagency.com/images/logo-mark-white.svg" alt="Melankolia" style="width:38px;height:auto;display:block;margin-bottom:22px"><div style="font-size:10px;letter-spacing:.28em;text-transform:uppercase;color:#c8a96e;font-weight:700">Melankolia Agency</div><h1 style="margin:8px 0 0;color:#fff;font-size:24px;line-height:1.15;font-weight:700">${esc(artist)} booking inquiry</h1><div style="margin-top:10px;color:#888;font-size:12px;letter-spacing:.08em;text-transform:uppercase">${esc(line([venue, city, country, date]))}</div></td></tr><tr><td style="padding:30px;color:#d7d7d7;font-size:15px"><p style="margin:0 0 18px">${esc(hello)}</p>${htmlBody}<p style="margin:28px 0 0;color:#aaa">Best wishes,<br><br><strong style="color:#fff">${esc(senderName)}</strong></p></td></tr></table></td></tr></table></body></html>`;

  // ---- Follow-up / nudge variant: short, warm "circling back" note ----
  if (String(body.emailType||'') === 'follow_up') {
    const fSubject = `Following up: ${artist} in ${venue || city}${datePhrase ? ` (${datePhrase})` : ''}`;
    const fOpen = `I hope you're doing well! I wanted to gently circle back on my earlier note about bringing ${artistWithOrigin} to ${venue || city}${country && !venue ? `, ${country}` : ''}${date ? ` around ${date}` : ` in ${datePhrase}`}.`;
    const fBody = `We're still keen to find a date that works, and I'd love to hear whether there's any availability or interest on your end${deal ? ` — the deal target is ${deal}, but we're flexible on structure to fit the room` : ''}. They could also slot into an existing event if you have something suitable around then.`;
    const fLinks = links.length ? `Happy to resend anything useful:` : '';
    const fText = `${hello}\n\n${fOpen}\n\n${fBody}${fLinks ? `\n\n${fLinks}\n${textLinks(links)}` : ''}\n\nNo rush at all — just wanted to keep it on your radar. Thanks so much!\n\nBest wishes,\n\n${senderName}`;
    const fHtmlBody = [
      `<p style="margin:0 0 18px">${esc(fOpen)}</p>`,
      `<p style="margin:0 0 18px">${esc(fBody)}</p>`,
      links.length ? `${`<p style="margin:0 0 8px">${esc('Happy to resend anything useful:')}</p>`}<ul style="margin:0 0 22px;padding-left:20px;color:#d7d7d7">${htmlLinks(links)}</ul>` : '',
      `<p style="margin:0 0 18px">No rush at all — just wanted to keep it on your radar. Thanks so much!</p>`
    ].filter(Boolean).join('');
    const fHtml = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(fSubject)}</title></head><body style="margin:0;background:#050505;color:#d7d7d7;font-family:Helvetica,Arial,sans-serif;line-height:1.55"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#050505;padding:32px 16px"><tr><td align="center"><table role="presentation" width="100%" style="max-width:720px;border:1px solid #222;background:#0b0b0b" cellspacing="0" cellpadding="0"><tr><td style="padding:28px 30px;border-bottom:1px solid #202020"><img src="https://melankoliaagency.com/images/logo-mark-white.svg" alt="Melankolia" style="width:38px;height:auto;display:block;margin-bottom:22px"><div style="font-size:10px;letter-spacing:.28em;text-transform:uppercase;color:#c8a96e;font-weight:700">Melankolia Agency</div><h1 style="margin:8px 0 0;color:#fff;font-size:22px;line-height:1.15;font-weight:700">Following up — ${esc(artist)}</h1><div style="margin-top:10px;color:#888;font-size:12px;letter-spacing:.08em;text-transform:uppercase">${esc(line([venue, city, country, date]))}</div></td></tr><tr><td style="padding:30px;color:#d7d7d7;font-size:15px"><p style="margin:0 0 18px">${esc(hello)}</p>${fHtmlBody}<p style="margin:28px 0 0;color:#aaa">Best wishes,<br><br><strong style="color:#fff">${esc(senderName)}</strong></p></td></tr></table></td></tr></table></body></html>`;
    return json(200, { success:true, data:{ subject:fSubject, text:fText, html:fHtml, preview: line([artist, venue, city, date]), follow_up:true, links_used: links.map(([label])=>label) } });
  }
