...
const BADT = /\b(me at the zoo|rick astley|never gonna give you up|official trailer|movie trailer|film trailer|teaser trailer|reaction|react|interview|podcast|tutorial|how to|unboxing|full album|full movie|lyric video|audiobook|documentary trailer|behind the scenes|makeup|vlog|gameplay|walkthrough|explained|review)\b/i;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const videos = [];
    const GOODT = /(official (music )?video|music video|official video|\bmv\b|live at|live in|live session|\(live\)|\[live\]|visualizer|lyric video|audio|full set|dj set|performance)/i;
    const vrel = (t) => { const n = norm(t); if (!n.includes(norm(artist))) return false; if (BADT.test(t)) return false; return true; };
    const vstrong = (t) => vrel(t) && GOODT.test(t);
    async function addY(id, strict) { if (!/^[A-Za-z0-9_-]{11}$/.test(id) || BAD.has(id) || videos.some(v => v.id === id)) return; try { const r = await fetch(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${id}&format=json`, { signal: signalFor(1800) }); if (!r.ok) return; const d = await r.json(), title = d.title || artist; if (!vrel(title)) return; if (strict && !vstrong(title)) return; videos.push({ id, title, thumb:`https://img.youtube.com/vi/${id}/mqdefault.jpg` }); } catch {} }
    if (!nearDeadline(5000)) await Promise.allSettled((p.youtubeVideoIds || []).map(x => yid(String(x))).filter(Boolean).slice(0,4).map(addY));
    if (!videos.length && !nearDeadline(6000)) { for (const q of [`"${artist}" music video`,`"${artist}" official video`,`"${artist}" live`,`"${artist}" youtube`]) { if (videos.length >= 3 || nearDeadline(3500)) break; try { const r = await fetch(`https://www.youtube.com/results?search_query=${encodeURIComponent(q)}`, { headers:{'User-Agent':'Mozilla/5.0'}, signal: signalFor(3000) }); if (!r.ok) continue; const h = await r.text(); const ids = [...new Set([...h.matchAll(/"videoId":"([A-Za-z0-9_-]{11})"/g)].map(m=>m[1]).concat([...h.matchAll(/watch\?v=([A-Za-z0-9_-]{11})/g)].map(m=>m[1])))].slice(0,6); for (const id of ids) { if (videos.length >= 3 || nearDeadline(3500)) break; await addY(id, true); } } catch {} } }
