...
function renderCvProposals() {
  ...
  host.innerHTML = _cvProposals.map(p => {
    ...
    if (p.type === 'restructure') {
      ...
      const contactRow = p.after.new_contact ? `<div class="disc-venues"><b>Add promoter/buyer:</b>${cvContactRow(p.after.new_contact)}</div>` : '';
      body = `<div class="disc-sub">Contract counterparty: ${escapeHtml(ct.organization || ct.promoter_person_name || '')} ${ct.event_or_venue ? '&middot; ' + escapeHtml(ct.event_or_venue) : ''}</div>${fieldRows}${contactRow}`;
