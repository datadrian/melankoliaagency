...
(function(){
    ...
    root.innerHTML = `
      <section class="route-ops-shell">
        <aside class="route-library-panel">
          ...
          <button type="button" class="btn-primary route-full-btn" onclick="RouteAdmin.renderBuilder()">+ New Tour</button>
          ...
          <div class="route-list-head"><span>Tours</span><button type="button" onclick="RouteAdmin.init()" title="Refresh tours" aria-label="Refresh tours">↻</button></div>
        ...
        <main class="route-ops-main">
          <div class="route-command-bar">
            ...
            <div class="route-command-actions">
              <details class="route-action-menu route-action-menu-right"><summary class="btn-secondary">Help</summary><div class="route-action-menu-panel"><button type="button" onclick="RouteAdmin.systemTour()">Quick tour</button><button type="button" onclick="window.open('/route-planner-guide.html','_blank')">Full guide</button></div></details>
            </div>
          </div>
          ...
          <section class="route-workflow-panel">
            ...
            <div class="route-workflow-steps">
              ...
              <div><b>04</b><strong>Confirm + advance</strong><span>Autosave creates the tour and its show records for advancing.</span></div>
            </div>
          </section>
        </main>
      </section>`;
    ...
    $('routeAdminShell').innerHTML = `
      <section class="route-plan-shell">
        <div class="route-plan-topbar">
          ...
          <div><p class="route-kicker">New routing session</p><h1>Plan a tour around the map</h1><span>Start with the essentials, then open travel and logistics only when they affect the route.</span></div>
          <details class="route-action-menu route-action-menu-right"><summary class="btn-secondary btn-sm">Help</summary><div class="route-action-menu-panel"><button type="button" onclick="RouteAdmin.systemTour()">Quick tour</button><button type="button" onclick="window.open('/route-planner-guide.html','_blank')">Full guide</button></div></details>
        </div>
        <div class="route-plan-grid">
          ...
          <aside class="route-control-panel">
            <form id="routeBuildForm" onsubmit="RouteAdmin.generate(event)">
              ...
              <details class="route-form-disclosure">
                <summary><span>Travel, gear &amp; transport</span><small>Optional — open when logistics affect the route</small></summary>
                <div class="route-form-disclosure-body">
              ...
              <label>Confirmed Anchors & Holds <small>one per line: City | Date | Venue | Deal | Status. Use hold, offer, deal_made, confirmed, or advanced.</small><textarea id="rtAnchors" class="form-input form-textarea" rows="5" placeholder="Berlin | 2026-10-16 | Urban Spree | €800 guarantee | hold">${esc(seed.anchorText||'')}</textarea></label>
              <div class="route-builder-actions route-builder-actions-focused"><button class="btn-primary" type="submit">Generate Route</button><details class="route-action-menu"><summary class="btn-secondary">More</summary><div class="route-action-menu-panel"><button type="button" onclick="RouteAdmin.analyzeAnchors()">Analyze anchors first</button></div></details></div>
            </form>
          </aside>
        </div>
        <div class="route-output-grid">
          <section class="route-output-panel"><div class="route-panel-title"><span>Generated route</span><em>Generation autosaves; review the logic before outreach.</em></div><div id="routeGeneratedResult">${routeStartHelp()}</div></section>
  ...
  function legRow(l,i){
    ...
    return `<article class="route-leg ${l.day_off?'day-off':''}" onclick="RouteAdmin.openStop(${i})">
      ...
      <div class="route-leg-actions" onclick="event.stopPropagation()">${!l.day_off?`<button class="btn-secondary btn-sm" onclick="RouteAdmin.openStop(${i})">Open stop</button><details class="route-action-menu"><summary class="btn-secondary btn-sm">More</summary><div class="route-action-menu-panel"><button type="button" onclick="RouteAdmin.suggestVenues(${i})" title="Instant CRM matches">CRM contacts</button><button type="button" onclick="RouteAdmin.venueFinderForStop(${i},event)" title="Live web search, ~20s">Search web</button><button type="button" onclick="RouteAdmin.generateEmail(${i})">Draft email</button><button type="button" onclick="RouteAdmin.adviseDeal(${i})">Deal advice</button><button type="button" onclick="RouteAdmin.backlineForStop(${i},{evt:event})" title="Live web search, ~30-60s">Backline research</button></div></details>`:`<button class="btn-secondary btn-sm" onclick="RouteAdmin.openStop(${i})">Open day</button>`}</div>
    </article>`;
  }
  ...
  function renderWorkbench(t){ return `
    <div class="route-workbench-disclosures">
      <details class="route-workbench-group"><summary><span>Plan &amp; analyze</span><small>4 tools</small></summary><div class="route-workbench-grid">
        <button onclick="RouteAdmin.reviewCurrentRoute()"><strong>AI oversight</strong><span>Review risks and propose fixes.</span></button>
        <button onclick="RouteAdmin.optimizeCurrent()"><strong>Optimize route</strong><span>Reorder without losing holds or deals.</span></button>
        <button onclick="RouteAdmin.estimateBudget()"><strong>Estimate budget</strong><span>Rates, costs, and break-even.</span></button>
        <button onclick="RouteAdmin.analyzeCurrentAnchors()"><strong>Analyze pipeline</strong><span>Find gaps and weak holds.</span></button>
      </div></details>
      <details class="route-workbench-group"><summary><span>Venues &amp; booking</span><small>2 tools</small></summary><div class="route-workbench-grid">
        <button onclick="RouteAdmin.renderVenueBoard()"><strong>Venue board</strong><span>Find, add, select, and email venues.</span></button>
        <button onclick="RouteAdmin.chatAgent()"><strong>Ask booking AI</strong><span>Routing, buyer, hold, and deal strategy.</span></button>
      </div></details>
      <details class="route-workbench-group"><summary><span>Travel &amp; logistics</span><small>4 tools</small></summary><div class="route-workbench-grid">
        <button onclick="RouteAdmin.backlineAllStops()"><strong>Backline</strong><span>Research suppliers and venue gear.</span></button>
        <button onclick="RouteAdmin.renderTravelHotelModule()"><strong>Travel hub</strong><span>Flights, trains, drives, and hotels.</span></button>
        <button onclick="RouteAdmin.renderTravelOpsBoard()"><strong>Travel ops</strong><span>Upcoming moves and confirmations.</span></button>
        <button onclick="RouteAdmin.renderTravelAlertCenter()"><strong>Travel alerts</strong><span>Resolve urgent travel risks.</span></button>
      </div></details>
    </div>
  ...
  function openStop(idx, opts={}){
    ...
    out.innerHTML = `<div class="route-tool-card route-stop-detail"><h3>Stop Detail — ${esc(l.city || 'TBD')}</h3>
      ...
      <div class="route-stop-actions"><button class="btn-primary" onclick="RouteAdmin.saveStopEdits(${idx})">Save stop</button>${!l.day_off?`<button class="btn-secondary" onclick="RouteAdmin.generateEmail(${idx})">Draft email</button>`:''}<details class="route-action-menu"><summary class="btn-secondary">More</summary><div class="route-action-menu-panel">${!l.day_off?`<button type="button" onclick="RouteAdmin.manualVenueForm(${idx})" title="Add a contact you already have">Add contact manually</button><button type="button" onclick="RouteAdmin.venueFinderForStop(${idx},event)" title="Live web search, ~20s">Search web for contacts</button><button type="button" onclick="RouteAdmin.backlineForStop(${idx},{evt:event})" title="Live web search, ~30-60s">Backline research</button><button type="button" onclick="RouteAdmin.insertBlankDayAfter(${idx})">Add blank day after</button>`:`<button type="button" onclick="RouteAdmin.convertBlankDayToProspect(${idx})">Convert to prospect</button><button type="button" class="danger" onclick="RouteAdmin.removeBlankDay(${idx})">Remove blank day</button>`}</div></details></div>
  ...
  function opsBoardRow(r,i){
    const next=r.next_travel||{}; const hotel=r.next_hotel||{};
    const isOk = !(r.flags||[]).length;
    const head = `<summary class="route-object-card-summary"><b>${esc([r.date,r.city,r.country].filter(Boolean).join(' · '))}</b><span class="rocs-meta">${esc([r.tour_name,r.artist,r.venue_name,r.status].filter(Boolean).join(' · '))}</span>${isOk?'<span class="ops-ok-tag">✓ On track</span>':`<span class="ops-risk-tag ${riskClass(r.risk_level)}">${esc((r.risk_level||'tight').toUpperCase())} · ${r.flags.length} flag${r.flags.length>1?'s':''}</span>`}</summary>`;
    const body = `<p><strong>Next move:</strong> ${esc(next.id?[humanMode(next.leg_type),next.origin_name||'origin',next.destination_name?`→ ${next.destination_name}`:'',next.departure_datetime,next.duration_minutes?`${next.duration_minutes} min`:''].filter(Boolean).join(' · '):'No travel saved')}</p><p><strong>Hotel:</strong> ${esc(hotel.id?[hotel.hotel_name||hotel.address||'Hotel',hotel.booking_status||''].filter(Boolean).join(' · '):'No hotel saved')}</p>${(r.flags||[]).length?`<ul>${r.flags.map(f=>`<li><strong>${esc(f.level)}</strong> — ${esc(f.message)}</li>`).join('')}</ul>`:'<p>No current travel/hotel flags.</p>'}<div class="route-stop-actions"><button class="btn-secondary btn-sm" onclick="RouteAdmin.openTour('${attr(r.tour_id)}')">Open Tour</button><button class="btn-primary btn-sm" onclick="RouteAdmin.openTourThenTravel('${attr(r.tour_id)}')">Plan Travel / Hotel</button>${next.id?`<button class="btn-secondary btn-sm" onclick="RouteAdmin.opsRouteLinks(${i})">Route Links</button>`:''}</div>`;
    return `<details class="route-object-card ${riskClass(r.risk_level)}" ${isOk?'':'open'}>${head}${body}</details>`;
  }
  function alertRangeButtons(active){ const opts=[['next2','Today + Tomorrow'],['today','Today'],['tomorrow','Tomorrow'],['week','7 Days'],['upcoming','Upcoming']]; return `<div class="route-stop-actions">${opts.map(([m,l])=>`<button class="${m===active?'btn-primary':'btn-secondary'} btn-sm" onclick="RouteAdmin.renderTravelAlertCenter('${m}')">${esc(l)}</button>`).join('')}</div>`; }
  ...
  function alertFilterButtons(){ const owners=['all','agency','promoter','band']; const levels=['all','critical','risky','tight']; return `<div class="route-filter-selects"><label>Owner<select class="form-input" onchange="RouteAdmin.setTravelAlertFilter('owner',this.value)">${owners.map(o=>`<option value="${o}" ${travelAlertOwnerFilter===o?'selected':''}>${esc(o==='all'?'All owners':o)}</option>`).join('')}</select></label><label>Severity<select class="form-input" onchange="RouteAdmin.setTravelAlertFilter('severity',this.value)">${levels.map(l=>`<option value="${l}" ${travelAlertSeverityFilter===l?'selected':''}>${esc(l==='all'?'All severities':l)}</option>`).join('')}</select></label></div>`; }
  ...
  const _ALERT_SEVERITY_RANK={critical:0,risky:1,tight:2}; function sortedVisibleAlerts(){ const visible=filteredTravelAlerts(); return [...visible].sort((a,b)=>{ const ar=a.status==='resolved'?1:0, br=b.status==='resolved'?1:0; if(ar!==br) return ar-br; const as=_ALERT_SEVERITY_RANK[a.level]??3, bs=_ALERT_SEVERITY_RANK[b.level]??3; if(as!==bs) return as-bs; return String(a.date||'').localeCompare(String(b.date||'')); }); }
  async function renderTravelAlertCenter(mode='week',includeResolved=false){ travelAlertMode=mode||travelAlertMode||'week'; travelAlertIncludeResolved=!!includeResolved; const out=toolOut(); out.innerHTML=loading('Loading travel alert center…'); try{ const data=await post(TRAVEL_API,{action:'getTravelAlerts',mode:travelAlertMode,include_resolved:travelAlertIncludeResolved}); const c=data.counts||{}, range=data.range||{}; travelAlertRows=data.alerts||[]; const visible=sortedVisibleAlerts(); out.innerHTML=`<div class="route-tool-card route-travel-module"><h3>Travel Alert Center</h3><p>${esc(range.label||'Next 7 days')} issue queue for travel, hotels, confirmations, and timing risks — most urgent first. <span class="route-muted">${esc([range.from,range.through].filter(Boolean).join(' → '))}</span></p>${alertRangeButtons(range.mode||travelAlertMode)}${alertResolvedToggle()}${alertFilterButtons()}<div class="route-stop-actions"><button class="btn-secondary btn-sm" onclick="RouteAdmin.copyTravelAlertDigest()">Copy Alert Digest</button></div><div class="ops-stat-grid">${statTile('Open',c.open||0)}${statTile('Critical',c.critical||0)}${statTile('Risky',c.risky||0)}${statTile('Tight',c.tight||0)}${statTile('Resolved',c.resolved||0)}</div><div class="route-object-card-list">${visible.length?visible.map(a=>alertRow(a,travelAlertRows.indexOf(a))).join(''):'<div class="route-ai-empty">No matching travel alerts in this range/filter.</div>'}</div></div>`;}catch(e){out.innerHTML=errorBox('Travel Alert Center failed',e.message);} }
  function alertRow(a,i){
    const owner=a.owner||'agency';
    return `<article class="route-object-card ${riskClass(a.level)}"><b>${esc(String(a.level||'tight').toUpperCase())} — ${esc([a.date,a.city,a.country].filter(Boolean).join(' · '))}</b><span>${esc([a.tour_name,a.artist,a.venue_name].filter(Boolean).join(' · '))}</span><p>${esc(a.message)}</p><p><strong>Owner:</strong> ${esc(owner)} · <strong>Status:</strong> ${esc(a.status||'open')}</p>${a.notes?`<p><strong>Notes:</strong> ${esc(a.notes)}</p>`:''}<div class="route-stop-actions">${a.status==='resolved'?`<button class="btn-secondary btn-sm" onclick="RouteAdmin.updateTravelAlert(${i},'open')">↺ Reopen</button>`:`<button class="btn-primary btn-sm" onclick="RouteAdmin.updateTravelAlert(${i},'resolved')">✓ Resolve</button>`}<button class="btn-secondary btn-sm" onclick="RouteAdmin.openTour('${attr(a.tour_id)}')">Open tour</button><details class="route-action-menu"><summary class="btn-secondary btn-sm">More</summary><div class="route-action-menu-panel"><button type="button" onclick="RouteAdmin.updateTravelAlert(${i},'open','agency')">${owner==='agency'?'✓ ':''}Assign agency</button><button type="button" onclick="RouteAdmin.updateTravelAlert(${i},'open','promoter')">${owner==='promoter'?'✓ ':''}Assign promoter</button><button type="button" onclick="RouteAdmin.updateTravelAlert(${i},'open','band')">${owner==='band'?'✓ ':''}Assign band</button><button type="button" onclick="RouteAdmin.openTourThenTravel('${attr(a.tour_id)}')">Plan travel</button><button type="button" onclick="RouteAdmin.editTravelAlertNote(${i})">Add note</button><button type="button" onclick="RouteAdmin.generateTravelAlertMessage(${i},'promoter_reminder')">Promoter draft</button><button type="button" onclick="RouteAdmin.generateTravelAlertMessage(${i},'band_note')">Band note</button><button type="button" onclick="RouteAdmin.generateTravelAlertMessage(${i},'agency_note')">Agency note</button></div></details></div></article>`;
  }
  ...
  const _OPS_SEVERITY_RANK={critical:0,risky:1,tight:2,none:3}; async function renderTravelOpsBoard(mode='next2'){ const out=toolOut(); out.innerHTML=loading('Loading travel ops board…'); try{ const data=await post(TRAVEL_API,{action:'getTravelOpsBoard',mode}); const c=data.counts||{}, range=data.range||{}; const sortedRows=[...(data.rows||[])].sort((a,b)=>(_OPS_SEVERITY_RANK[a.risk_level]??4)-(_OPS_SEVERITY_RANK[b.risk_level]??4)); opsBoardRows=sortedRows; out.innerHTML=`<div class="route-tool-card route-travel-module"><h3>Travel Ops Board</h3><p>${esc(range.label||'Today + tomorrow')} view across active tours: next movement, hotel status, missing records, and timing risk — most urgent first, on-track shows collapsed. <span class="route-muted">${esc([range.from,range.through].filter(Boolean).join(' → '))}</span></p>${opsRangeButtons(range.mode||mode)}<div class="ops-stat-grid">${statTile('Shows',c.shows||0)}${statTile('Critical',c.critical||0)}${statTile('Risk/Tight',(c.risky||0)+(c.tight||0))}${statTile('Missing Travel',c.missing_travel||0)}${statTile('Missing Hotels',c.missing_hotels||0)}</div><div class="route-object-card-list">${sortedRows.length?sortedRows.map(opsBoardRow).join(''):`<div class="route-ai-empty">No active shows found for ${esc((range.label||'this range').toLowerCase())}.</div>`}</div></div>`;}catch(e){out.innerHTML=errorBox('Travel Ops Board failed',e.message);} }
  ...
  function renderVenueBoard(){
    ...
    if(!t?.legs?.length){ out.innerHTML=errorBox('No active route','Generate or open a route first.'); return; }
    const stops=t.legs.map((l,i)=>({l,i})).filter(x=>!x.l.day_off);
    out.innerHTML=`<div class="route-tool-card route-venue-board"><div class="route-tool-head"><h3>Venue Board</h3><div class="route-stop-actions"><button class="btn-secondary btn-sm" onclick="RouteAdmin.renderTourOutreach()" title="Track outreach stage, confirm, and follow up">⇄ Outreach Board</button></div></div><p>Every city's candidates side by side. Find and select the primary venue here; use Outreach for contact status, emails, and follow-ups.</p><div class="route-stop-actions"><button class="btn-primary" onclick="RouteAdmin.researchVenuesAllStops()">Research All Cities</button></div>${stops.map(({l,i})=>{
      const vs=Array.isArray(l.candidate_venues)?l.candidate_venues:[];
      ...
      return `<section class="route-venue-city"><div class="route-venue-city-head"><div><b>${esc(i+1)}. ${esc(l.city||'TBD')}</b><span>${esc([l.date,l.country].filter(Boolean).join(' · '))}</span>${summary}</div><div class="route-venue-city-actions"><button class="btn-secondary btn-sm" onclick="RouteAdmin.venueFinderForStop(${i},event)" title="Live web search, ~20s">Search web</button><details class="route-action-menu route-action-menu-right"><summary class="btn-secondary btn-sm">More</summary><div class="route-action-menu-panel"><button type="button" onclick="RouteAdmin.manualVenueForm(${i})">Add contact manually</button><button type="button" onclick="RouteAdmin.openStop(${i})">Open stop detail</button></div></details></div></div>${shown.length?shown.map((v,vi0)=>{ const vi=vs.indexOf(v); const st=vStatus(v); return `<div class="route-venue-row"><div class="ob-row-head"><strong>${esc(v.name||'Venue')}</strong><span class="ob-pill ob-pill-${st}">${esc(OUTREACH_LABEL[st]||st)}</span></div><span>${esc(venueMeta(v))}${v.website?` · <a class="route-inline-link" href="${attr(v.website)}" target="_blank">Website</a>`:''}</span><p>${esc(v.fit_reason||v.reason||v.outreach_angle||'')}</p><div class="route-venue-actions"><button class="btn-primary btn-sm" onclick="RouteAdmin.useCandidateVenue(${i},${vi})" title="Set as this stop's primary pick">Select venue</button></div></div>`; }).join(''):'<div class="route-ai-empty">No venue options yet. Use Search web or More → Add contact manually.</div>'}${vs.length>8?`<p class="route-muted route-help-inline">+${vs.length-8} more — see the <a href="javascript:void 0" onclick="RouteAdmin.openStop(${i})">full Stop Detail</a>.</p>`:''}</section>`;
    }).join('')}</div>`;
  }
    ...
    return `<div class="ob-row ob-${st}">
      ...
      <div class="ob-actions">
        ${confirmed
          ...
          : `<label class="ob-status">Status <select class="form-input ob-select" onchange="RouteAdmin.setVenueOutreach(${idx},${vi},this.value)">${opts}</select></label><button class="btn-primary btn-sm" onclick="RouteAdmin.confirmVenue(${idx},${vi})">Confirm</button>`}
        ${v.crm_matched?'<span class="ob-crm-tag">✓ CRM</span>':''}
        <details class="route-action-menu"><summary class="btn-secondary btn-sm">More</summary><div class="route-action-menu-panel">${!confirmed?`<button type="button" onclick="RouteAdmin.logVenueContacted(${idx},${vi})">Log contacted today</button>`:''}<button type="button" onclick="RouteAdmin.generateVenueEmail(${idx},${vi})">Draft branded email</button><button type="button" onclick="RouteAdmin.venueOutreachNote(${idx},${vi})">Add note</button>${(!v.email && (!v.booking_method||v.booking_method==='unknown') && !v.booking_form_url)?`<button type="button" onclick="RouteAdmin.autofillVenueFromCRM(${idx},${vi})">Fill from CRM</button>`:''}${v.website?`<a href="${attr(v.website)}" target="_blank">Open website</a>`:''}</div></details>
      </div></div>`;
  }
  function renderOutreachBoard(l,idx){
    const list=Array.isArray(l.candidate_venues)?l.candidate_venues:[];
    if(!list.length) return `<div class="route-stop-venues"><h4>Venue outreach</h4><div class="route-ai-empty">No venues attached yet. Use CRM contacts, Search web, or Add contact manually to build this date’s outreach list.</div></div>`;
  ...
  function venueCard(v,i){ const contactsHtml = Array.isArray(v.contacts)&&v.contacts.length ? `<div class="vm-contacts">${v.contacts.map(function(ct){ return `<span class="vm-contact-chip">${esc(ct.name||'(unnamed)')}${ct.title?' · '+esc(ct.title):''}${ct.email?' · '+esc(ct.email):''}${ct.phone?' · '+esc(ct.phone):''}${ct.is_primary?' <b>★</b>':''}</span>`; }).join('')}</div>` : ''; return `<article class="venue-manager-card"><div><strong>${esc(v.name||'Venue')}</strong><span>${esc([v.contact_type||'contact',v.city,v.country,v.capacity?`cap ${v.capacity}`:'cap unknown',venueRating(v.rating)].filter(Boolean).join(' · '))}</span><p>${esc(v.notes||v.rag_text||'')}</p><em>${esc([v.booking_email||v.booking_method,v.phone,v.relationship_status].filter(Boolean).join(' · '))}</em>${contactsHtml}</div><div class="route-stop-actions"><button class="btn-secondary btn-sm" onclick="VenueManager.edit(${i})">Edit</button><button class="btn-secondary btn-sm" onclick="VenueManager.sendToRoute(${i})">Send To Current Route</button><button class="btn-secondary btn-sm" onclick="VenueManager.enrich(${i},false)" title="Fast: Gemini web lookup for missing fields (~3s)">⚡ Fast find</button><button class="btn-secondary btn-sm" onclick="VenueManager.enrich(${i},true)" title="Deep: also opens the Linktree / website and scrapes it (~10s)">🔎 Deep find</button>${v.website?`<a class="btn-secondary btn-sm" href="${attr(v.website)}" target="_blank">Site</a>`:''}</div></article>`; }
