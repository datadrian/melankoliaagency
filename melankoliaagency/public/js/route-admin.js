...
(function(){
        ...
        <main class="route-ops-main">
          <div class="route-command-bar">
            ...
            <div class="route-command-actions">
              ...
              <button type="button" class="btn-secondary" onclick="window.open('/route-planner-guide.html','_blank')">Full Guide</button>
  ...
  function alertFilterButtons(){ const owners=['all','agency','promoter','band']; const levels=['all','critical','risky','tight']; return `<div class="route-stop-actions"><span class="route-muted route-help-inline">Owner</span>${owners.map(o=>`<button class="${travelAlertOwnerFilter===o?'btn-primary':'btn-secondary'} btn-sm" onclick="RouteAdmin.setTravelAlertFilter('owner','${o}')">${esc(o==='all'?'All':o)}</button>`).join('')}<span class="route-btn-divider"></span><span class="route-muted route-help-inline">Severity</span>${levels.map(l=>`<button class="${travelAlertSeverityFilter===l?'btn-primary':'btn-secondary'} btn-sm" onclick="RouteAdmin.setTravelAlertFilter('severity','${l}')">${esc(l==='all'?'All':l)}</button>`).join('')}</div>`; }
  ...
  function alertRow(a,i){
    ...
    return `<article class="route-object-card ${riskClass(a.level)}"><b>${esc(String(a.level||'tight').toUpperCase())} — ${esc([a.date,a.city,a.country].filter(Boolean).join(' · '))}</b><span>${esc([a.tour_name,a.artist,a.venue_name].filter(Boolean).join(' · '))}</span><p>${esc(a.message)}</p><p><strong>Owner:</strong> ${esc(owner)} · <strong>Status:</strong> ${esc(a.status||'open')}</p>${a.notes?`<p><strong>Notes:</strong> ${esc(a.notes)}</p>`:''}<div class="route-stop-actions">${a.status==='resolved'?`<button class="btn-secondary btn-sm" onclick="RouteAdmin.updateTravelAlert(${i},'open')">↺ Reopen</button>`:`<button class="btn-primary btn-sm" onclick="RouteAdmin.updateTravelAlert(${i},'resolved')">✓ Mark Resolved</button>`}<span class="route-btn-divider"></span><span class="route-muted route-help-inline">Assign</span><button class="${owner==='agency'?'btn-primary':'btn-secondary'} btn-sm" onclick="RouteAdmin.updateTravelAlert(${i},'open','agency')">Agency</button><button class="${owner==='promoter'?'btn-primary':'btn-secondary'} btn-sm" onclick="RouteAdmin.updateTravelAlert(${i},'open','promoter')">Promoter</button><button class="${owner==='band'?'btn-primary':'btn-secondary'} btn-sm" onclick="RouteAdmin.updateTravelAlert(${i},'open','band')">Band</button><span class="route-btn-divider"></span><button class="btn-secondary btn-sm" onclick="RouteAdmin.openTour('${attr(a.tour_id)}')">Open Tour</button><button class="btn-secondary btn-sm" onclick="RouteAdmin.openTourThenTravel('${attr(a.tour_id)}')">Plan Travel</button><span class="route-btn-divider"></span><button class="btn-secondary btn-sm" onclick="RouteAdmin.editTravelAlertNote(${i})">Add Note</button><button class="btn-secondary btn-sm" onclick="RouteAdmin.generateTravelAlertMessage(${i},'promoter_reminder')">Promoter Draft</button><button class="btn-secondary btn-sm" onclick="RouteAdmin.generateTravelAlertMessage(${i},'band_note')">Band Note</button><button class="btn-secondary btn-sm" onclick="RouteAdmin.generateTravelAlertMessage(${i},'agency_note')">Agency Note</button></div></article>`;
  }
